 ...
 * wrote both, so git stored them as one blob and the startup profile carried nothing the baseline
 * profile did not -- dex layout had nothing to differentiate on. [startup] now feeds the startup
 * profile alone, so regenerating moves the two files by very different amounts: roughly -285 lines
 * for startup and +7,400 for baseline. That diff is the fix working, not a broken generator.
 **/
