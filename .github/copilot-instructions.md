...
- Encode `versionCode` as the single-digit major, minor, and patch components followed by a two-digit build counter (`MmpBB`). For a new `versionName`, start the `versionCode` build counter at `00` (for example, `versionName` `1.7.1` maps to `versionCode` `17100`); additional builds of that version increment the suffix (`17101`, `17102`, and so on).
...
- Keep release signing, Play API, Firebase, and Google Services files out of git. Release builds read the private Google Services file directly from `APPWATCHER_GOOGLE_SERVICES_FILE` in the user's Gradle user home `gradle.properties` (the `GRADLE_USER_HOME` directory, defaulting to `~/.gradle` or `%USERPROFILE%\.gradle` on Windows), alongside signing settings such as `APPWATCHER_KEYSTORE_FILE`, `APPWATCHER_KEYSTORE_PASSWORD`, `APPWATCHER_KEY_ALIAS`, and `APPWATCHER_KEY_PASSWORD`; do not print private values in logs or copy them into tracked files.
...
- One-shot `viewActions` collectors should use stable `LaunchedEffect` keys such as the view model or `Unit`. For changing callbacks like navigation lambdas, wrap them with `rememberUpdatedState` and call the current value from inside the collector instead of keying the collector by the lambda.
...
- Watchlist paging uses a custom raw-query `PagingSource`, not Room-generated paging. Keep app-list ordering deterministic with a stable row-id tie-breaker, and bridge `AppListTable.Queries.changes(db.apps())` to invalidate cached pager factories when app rows mutate.
- Duplicate Lazy/Paging keys in watchlist-style lists usually indicate duplicate data emitted within one paging generation. Do not hide this by adding volatile values to Compose keys; fix ordering, invalidation, or section key construction at the paging/data layer.
...
- Preserve existing Kotlin formatting. Do not run broad `ktlintFormat`/autoformat over source files unless explicitly requested; keep existing multiline constructor/property declarations and avoid collapsing multiline data classes just to satisfy formatting tools. Declaration parameters/properties may stay on their own lines, short inheritance lists should stay inline when readable, and boolean operators such as `&&`/`||` should not be split onto standalone lines.
- The project intentionally disables several ktlint wrapping rules in `.editorconfig` to preserve manual wrapping style; do not re-enable them or reformat files broadly unless the task is explicitly about changing formatting policy.
- `.editorconfig` sets `insert_final_newline = false`; avoid adding final newlines when touching files where ktlint enforces this setting.
- In Compose rows, `horizontalArrangement = Arrangement.Center` only centers within the row's measured width. If content must be centered within a cell or column, give the row an explicit width such as `Modifier.fillMaxWidth()` or use an equivalent parent alignment.
