...
interface AppListTable {
    ...
    fun observeRows(query: RoomRawQuery): Flow<List<Int>>

    @RawQuery(observedEntities = [(App::class), (AppChange::class), (AppTag::class)])
    fun observe(query: RoomRawQuery): Flow<List<AppListItem>>

    @RawQuery(observedEntities = [(App::class), (AppChange::class), (AppTag::class)])
    suspend fun load(query: RoomRawQuery): List<AppListItem>
    ...
    suspend fun loadRowIds(packageNames: List<String>): List<PackageRowPair> {
        return packageNames.chunked(998).flatMap { _loadRowIds(it) }
    }
    ...
    @Query(
        "SELECT $table.*, " +
                "NULL AS ${ChangelogTable.Columns.details}, " +
                "FALSE AS ${ChangelogTable.Columns.noNewDetails}, " +
                "CASE WHEN ${Columns.syncTimestamp} > :recentTime THEN 1 ELSE 0 END ${Columns.recentFlag} " +
                "FROM $table WHERE " +
                "CASE :includeDeleted WHEN 0 THEN ${Columns.status} != ${App.STATUS_DELETED} ELSE ${Columns.status} >= ${App.STATUS_NORMAL} END "
    )
    fun load(includeDeleted: Boolean, recentTime: Long = AppListTable.recentTime): PagingSource<Int, AppListItem>
    ...
    fun loadAppList(includeDeleted: Boolean, sortId: Int, recentTime: Long = AppListTable.recentTime): PagingSource<Int, AppListItem>
    ...
    @Query("DELETE FROM $table WHERE ${Columns.appId} == :appId")
    @Transaction
    suspend fun delete(appId: String): Int
    ...
    // TODO: Skip id to apply autoincrement
    ...
    object Queries {
        ...
        fun changes(table: AppListTable): Flow<List<Int>> {
            return table.observeRows(
                query = RoomRawQuery(
                    sql = "SELECT ${BaseColumns._ID} FROM ${AppListTable.table} LIMIT 1"
                )
            )
        }
        ...
        private fun createAppsListQuery(
            ...
            val sql =
                "SELECT $table.*, ${ChangelogTable.TableColumns.details}, ${ChangelogTable.TableColumns.noNewDetails}, " +
                        ...
                        rangeSql
            return RoomRawQuery(sql, onBindStatement = {
                selection.second.forEachIndexed { index, value ->
                    it.bindText(index, value)
                }
            })
        }
