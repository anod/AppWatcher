...
private fun ScheduleRow(schedule: Schedule, dateFormat: DateFormat) {
    ...
    Column(Modifier.padding(16.dp)) {
        ...
        FlowRow(
            ...
            ScheduleChip(
                text = when (result) {
                    is ScheduleResult.New -> stringResource(R.string.schedule_status_new)
                    is ScheduleResult.Success -> stringResource(R.string.schedule_status_success)
                    is ScheduleResult.Failed -> stringResource(R.string.schedule_status_failed)
                    is ScheduleResult.Skipped -> stringResource(R.string.schedule_status_skipped)
                },
                color = when (result) {
                    is ScheduleResult.New -> colorBlue
                    is ScheduleResult.Success -> colorGreen
                    is ScheduleResult.Failed -> colorOrange
                    is ScheduleResult.Skipped -> colorGrey
                }
            )
        ...
        if (result is ScheduleResult.Failed || result is ScheduleResult.Skipped) {
            Text(text = when (result) {
                is ScheduleResult.Failed -> when (result.reason) {
                    Schedule.statusFailed -> "Unknown error"
                    Schedule.statusFailedNoToken -> "Cannot receive access token"
                    else -> ""
                }
                is ScheduleResult.Skipped -> when (result.reason) {
