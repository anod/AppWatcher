...
class UpdateCheck(
    ...
    private val androidPreferences: Preferences,
    ...
    suspend fun perform(extras: Data): Int = withContext(Dispatchers.Default) {
        ...
        val account = androidPreferences.account?.toAndroidAccount()
        if (account == null) {
        ...
        if (!manualSync) {
            if (androidPreferences.isWifiOnly && !networkConnection.isWifiEnabled) {
            ...
            val updateTime = androidPreferences.lastUpdateTime
        ...
        val lastUpdatesViewed = androidPreferences.isLastUpdatesViewed
        ...
        androidPreferences.lastUpdateTime = now

        if (!manualSync
                && updatedApps.isNotEmpty()
                && (updatedApps.firstOrNull { it.isNewUpdate } != null)
                && lastUpdatesViewed) {
            androidPreferences.isLastUpdatesViewed = false
        }

        notifyIfNeeded(manualSync, updatedApps, schedule)

        if (!manualSync) {
            if (androidPreferences.isDriveSyncEnabled) {
                AppLog.d("DriveSyncEnabled = true")
                performGDriveSync(androidPreferences, now)
            } else {
                AppLog.d("DriveSyncEnabled = false, skipping...")
            }

            Cleanup(androidPreferences, database).performIfNeeded(now)
        }
    ...
    private suspend fun doSync(lastUpdatesViewed: Boolean): SyncResult {
        val sortId = androidPreferences.sortIndex
        val updatedApps = mutableListOf<UpdatedApp>()
        var unavailable = 0
        var total = 0
        database.apps().loadAppList(includeDeleted = false, sortId).asFlow(loadSize = BULK_SIZE)
            .onEach { list ->
                total += list.size
                val localApps = list.associateBy { it.app.packageName }
        ...
        AppLog.i("Sync finished for ${total} apps", "UpdateCheck")
        return SyncResult(true, updatedApps, total, unavailable)
    }
    ...
    private suspend fun notifyIfNeeded(manualSync: Boolean, updatedApps: List<UpdatedApp>, schedule: Schedule) {
        ...
        if (manualSync) {
            ...
            val filter = SyncNotificationFilter(androidPreferences)
