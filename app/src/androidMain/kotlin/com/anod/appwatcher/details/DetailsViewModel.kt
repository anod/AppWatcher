...
sealed interface ChangelogLoadState {
    data object Initial : ChangelogLoadState
    data object RemoteError : ChangelogLoadState
    data object Complete : ChangelogLoadState
}

sealed interface AppLoadingState {
    data object Initial : AppLoadingState
    data object Loaded : AppLoadingState
    data object NotFound : AppLoadingState
}

sealed interface AppIconState {
    data object Initial : AppIconState
    class Loaded(val bitmap: Bitmap) : AppIconState
    data object Default : AppIconState
}
...
class DetailsViewModel(
    ...
    private fun loadAppIcon(iconUrl: String) {
        viewModelScope.launch {
            val bitmap = iconLoader.get(iconUrl)?.toBitmap()
            if (bitmap == null) {
            ...
            viewState = viewState.copy(
                appIconState = AppIconState.Loaded(bitmap = bitmap),
    ...
    private fun changeTag(tagId: Int, checked: Boolean) {
        viewModelScope.launch {
            if (checked) {
                ...
                database.appTags().insert(AppTag(tagId = tagId, appId = viewState.appId))
            }
        }
    }
