...
class WishListEndpointPagingSource(private val dfeApi: DfeApi, installedApps: InstalledApps, uploadDateParserCache: UploadDateParserCache) :
    ListEndpointPagingSource(
        listType = DfeListType.ALL,
        installedApps = installedApps,
        appAdapter = { document -> document.toApp(uploadDateParserCache) }
    ) {
    override suspend fun execute(nextPageUrl: String): ResponseWrapper = dfeApi.wishlist(nextPageUrl)
}